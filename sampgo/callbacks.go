...
func onPlayerEditAttachedObject(playerid C.int, response C.int, index C.int, modelid C.int, boneid C.int, floats *C.float) bool {
	fSl := unsafe.Slice(floats, 9)
	...
	return fn(Player{ID: int(playerid)}, int(response), int(index), int(modelid), int(boneid), float32(fSl[0]), float32(fSl[1]), float32(fSl[2]), float32(fSl[3]), float32(fSl[4]), float32(fSl[5]), float32(fSl[6]), float32(fSl[7]), float32(fSl[8]))
}
